...
def chatgpt_description(api_base, api_key, analysis_YearMonth, table_data_str, model, disease_name = ''):
    ...
    data = {
        ...
        'max_tokens': 10000,
        'messages': [
            {"role": "user", "content": "I'm working on the results of my paper, playing a professional epidemiologist and  text generator to describe the data. Next, I will send you the monthly cases and deaths of different diseases. Please describe these data based on this data."},
            {"role": "system", "content": "OK, you can send me the data directly to me."},
            {"role": "user", "content": f"This is the monthly cases and deaths in {analysis_YearMonth} {disease_name}  in mainland, China, Please directly write the discussion of the data for paper."},
            {"role": "system", "content": "I will play a professional writer, create the discussion of the paper."},
            {"role": "user", "content": f"In the case of time series data, you can analyze seasonal and cyclical issues. Here is the data:\n{table_data_str}"}
        ]
    }
...
def chatgpt_mail_rebuild(api_base, api_key, mail_raw, model):
    url = f"{api_base}"
    headers = {
        'Content-Type': 'application/json',
        'Authorization': f'Bearer {api_key}'
    ...
    data = {
        ...
        'max_tokens': 10000,
        'messages': [
            {"role": "system", "content": mail_raw},
            {"role": "user", "content": "Good, Please indicate the email that the content was generated by ChatGPT. My name is Chinese Notifiable Infectious Diseases Sensing Project, you need replace. And add [Date] after my name."}
        ]
    }
    ...
    try:
        ...
        if response.status_code == 200:
            print('Rebuild Mail Success')
            out_content = response.json()['choices'][0]['message']['content']
            out_content = out_content.replace('Discussion:\n\n', '')
        else:
            print('Rebuild Mail Fail')
            print(response)
            out_content = None
    except ConnectionError as e:
        print('Connection Error:', e)
